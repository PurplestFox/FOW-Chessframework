...
namespace ChessChallenge.Chess
{
    ...
    public sealed class Board
    {
        ...
        public void GetFOWBoard(FenUtility.PositionInfo posInfo, API.Board fullBoard)
        {
            StartPositionInfo = posInfo;
            Initialize();

            //get all visible squares for player to move

            ulong playerToMovePieces = fullBoard.IsWhiteToMove ? fullBoard.WhitePiecesBitboard : fullBoard.BlackPiecesBitboard;
            ulong visibleSqaures = playerToMovePieces;

            while(playerToMovePieces > 0)
            {
            ...
            {
                int squareIndex = BitboardHelper.ClearAndGetIndexOfLSB(ref visibleSqaures);
                int piece = posInfo.squares[squareIndex];
                int pieceType = PieceHelper.PieceType(piece);
                int colourIndex = PieceHelper.IsWhite(piece) ? WhiteIndex : BlackIndex;
                Square[squareIndex] = piece;
